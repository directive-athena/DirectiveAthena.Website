...
public class WorldFaqRepositoryTests {
    ...
    private static WorldFaq CreateRule(int seed) => new() {
        Id = Guid.NewGuid(),
    ...
    public async Task GetRulesAsync_HandlesHttpClientFailure() {
        ...
        var logger = Substitute.For<ILogger<WorldFaqRepository>>();
        var repo = new WorldFaqRepository(storage, logger);

        // Act
        IEnumerable<WorldFaq> result = await repo.GetAllAsync();
    ...
    public async Task GetByIdAsync_ReturnsNullForSoftDeleted() {
        ...
        // Act
        WorldFaq? result = await repo.GetByIdAsync(rule.Id);
    ...
    public async Task SoftDeleteByIdAsync_MarksDeletedAndWritesIndex() {
        ...
        WorldFaq[]? saved = JsonSerializer.Deserialize<WorldFaq[]>(
            capturedJson!,
